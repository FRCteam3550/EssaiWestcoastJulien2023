 ...
 */
public class RamseteCommandWithTelemetry extends CommandBase implements ControlerTelemetryData {
  ...
  private DifferentialDriveWheelSpeeds m_prevRefSpeeds;
  private double m_prevLeftOutput;
  private double m_prevRightOutput;
   ...
   */
  public RamseteCommandWithTelemetry(
  ...
  public void execute() {
    ...
    var state = m_trajectory.sample(curTime);
    m_prevRefSpeeds = 
      m_kinematics.toWheelSpeeds(new ChassisSpeeds(state.velocityMetersPerSecond, 0, state.curvatureRadPerMeter * state.velocityMetersPerSecond));
    var targetWheelSpeeds =
      m_kinematics.toWheelSpeeds(m_follower.calculate(m_pose.get(), state));
    ...
    if (m_usePID) {
      ...
      m_prevLeftOutput = leftSpeedSetpoint;
      m_prevRightOutput = rightSpeedSetpoint;
    }

    m_output.accept(m_prevLeftOutput, m_prevRightOutput);
  ...
  public DifferentialDriveWheelSpeeds reference() {
    return m_prevRefSpeeds != null ? m_prevRefSpeeds : ControlerTelemetryData.ZERO_SPEEDS;
  }
  ...
  public Pair<Double, Double> output() {
    return new Pair<>(m_prevLeftOutput, m_prevRightOutput);
  }
}
